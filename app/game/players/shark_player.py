# app/game/players/shark_player.py
import random
import eventlet
...
from . import (timeSleep, _get_my_position, _card_rank_to_int,
               ...
               _get_pot_size, _is_strong_preflop_simple)  # Usamos el stub preflop por simplicidad
...
class SharkPlayer(BasePokerPlayer):
    def __init__(self, socketio, name, image,room_id):
        ...
        self.hole_card = []
        self.game_info = {}
        self.was_preflop_aggressor = False  # Nivel 6: Estado para C-Bet
        self.room_id=room_id

    # Reutilizar helper de niveles anteriores
    def _should_play_preflop(self, hole_card, position):
        # Podrías refinar esto aún más, pero usamos la base de Casual/Expert
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        print(f"--- {self.name} (Shark) acting ---")
        ...
        if street == 'preflop':
            ...
            if should_play:
                ...
                if will_raise:
                    ...
                    self.was_preflop_aggressor = True  # Marcar agresor
                elif call_action:
                    action_to_take = call_action
                elif check_action:
                    action_to_take = check_action
                elif raise_action:  # Si solo queda raise y no entramos en prob, lo tomamos igual
                    action_to_take = raise_action
            ...
            print(f"{self.name} Postflop Hand: {hand_strength}, Draw: {has_draw}, Pot: {pot_size}, PF Aggro: {self.was_preflop_aggressor}")
            is_value_hand = hand_strength in ["TWO_PAIR", "TRIPS_OR_BETTER"]
            is_decent_hand = hand_strength == "PAIR"

            # --- Nivel 6: Apuesta de Continuación (C-Bet) ---

            # Es flop? Fui agresor PF? Puedo pasar (nadie apostó antes)?
            if street == 'flop' and self.was_preflop_aggressor and check_action:
                if random.random() < 0.65:  # 65% C-Bet chance
                    print(f"{self.name} attempting C-Bet...")
                    action_to_take = raise_action  # Usamos raise para apostar
                    if action_to_take:
                        cbet_attempted = True
                        # Tamaño C-Bet (33-50% del bote) lo calcularemos en la sección Amount
                        print(f"{self.name} Planning C-Bet")
                    else:  # No se puede apostar? Raro
                        action_to_take = check_action  # Pasar si no puedo apostar

            # --- Lógica normal si no C-Beteamos o no aplica ---
            if not cbet_attempted:
                if is_value_hand:
                    ...
                    raise_prob = 0.10  # Raise raro con par
                    if raise_action and random.random() < raise_prob:
                        action_to_take = raise_action
                    elif call_action:
                        action_to_take = call_action
                    ...
                    # --- Nivel 6: Semi-Farol ---
                    semi_bluff_chance = 0.30
                    if self.was_preflop_aggressor:
                        semi_bluff_chance += 0.10

                    if raise_action and random.random() < semi_bluff_chance:
                        action_to_take = raise_action
                        print(f"{self.name} Semi-bluffing with draw!")
                    elif call_action:  # Considerar odds como en Expert
                        ...
                        reasonable_odds = pot_odds < 0.40  # Umbral un poco más loose
                        if reasonable_odds:
                            action_to_take = call_action
                        elif fold_action:
                            action_to_take = fold_action
        ...
        if action_to_take:
            ...
            if action_type == "raise":
                ...
                if min_raise == -1:  # Fallback
                    ...
                    target_bet_fraction = 0.5  # Default
                    if is_cbet:
                        target_bet_fraction = random.uniform(0.33, 0.50)
                    elif is_value_raise:
                        target_bet_fraction = random.uniform(0.55, 0.80)
                    elif is_semi_bluff:
                        target_bet_fraction = random.uniform(0.50, 0.70)
                    else:
                        target_bet_fraction = random.uniform(
                            0.40, 0.60)  # Otros raises
                    ...
                    if amount <= 0 and min_raise > 0:
                        amount = min_raise
                    print(f"{self.name} {'CBET' if is_cbet else 'Value' if is_value_raise else 'SemiBluff' if is_semi_bluff else 'Standard'} Raise - Target bet: {target_bet_fraction*100:.0f}% pot -> Amount: {amount}")
    ...
    def receive_round_start_message(self, round_count, hole_card, seats):
        ...
        self.was_preflop_aggressor = False  # Resetear estado
