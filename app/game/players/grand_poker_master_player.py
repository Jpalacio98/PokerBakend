# app/game/players/grand_master_player.py
import random
import eventlet
...
class GrandPokerMasterPlayer(BasePokerPlayer):
    ...
    # --- Helpers (copiados de niveles anteriores) ---
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        print(f"--- {self.name} (Grand Master) acting ---")
        ...
        if bb_amount == 0:
            bb_amount = 1  # Evitar división por cero
        my_stack_bb = my_stack / bb_amount
        print(f"{self.name} Stack: {my_stack} ({my_stack_bb:.1f} BB)")

        # FIX: Definir variable para premium preflop fuera del bloque if
        ...
        if street == 'preflop' and my_stack_bb < PUSH_FOLD_THRESHOLD_BB:
            ...
            if should_push and raise_action:
                ...
                if all_in_raise_amount >= my_stack:
                    amount = my_stack  # Nuestro stack completo
                else:  # Si max_raise < stack, ir all-in con lo que se pueda
        ...
        # --- Nivel 9: Explotación (Usar stats simuladas) ---
        # ... (código de explotación igual que antes) ...
        ...
        planning_check_raise_check = False  # Lógica Check/Raise (como GPM)
        # ... (código de planificación check/raise igual que antes) ...
        can_check_raise = (is_value_hand or (
            is_strong_draw and random.random() < 0.6))
        is_first_to_act = check_action is not None
        is_oop = True  # Simplificación
        if street != 'preflop' and is_first_to_act and is_oop and can_check_raise and random.random() < 0.25:
            action_to_take = check_action
            planning_check_raise_check = True
            print(
                f"{self.name} Planning check-raise check with {hand_strength or 'Draw'}")

        # --- Lógica de Acción Principal (si no push/fold, ni check/raise plan) ---
        if not planning_check_raise_check:
            if street == 'preflop':
                ...
                # FIX: Calcular y guardar el valor aquí
            ...
            else:  # Lógica Postflop (adaptada de NationalChampion con explotación)
                # ... (código postflop igual que antes, usando modificadores) ...
                ...
                if street == 'flop' and self.was_preflop_aggressor and check_action:
                    cbet_prob = 0.70 * exploit_modifier_cbet * aggression_modifier
                ...
                if not cbet_success:
                    if is_value_hand:
                        raise_prob = 0.85 * aggression_modifier
                        ...
                        call_prob_mod = exploit_modifier_bluff_catch
                        ...
                        semi_bluff_chance = 0.35 * exploit_modifier_cbet * aggression_modifier
                        ...
                        if street == 'river' and active_players == 2 and check_action and raise_action and random.random() < 0.10 * exploit_modifier_cbet:
                            action_to_take = raise_action
        ...
        if action_to_take:
            ...
            if action_type == "raise":
                ...
                if min_raise == -1:  # Fallback
                    ...
                    # FIX: Usar la variable is_premium_hand_preflop_calc definida antes
                    ...
                    target_bet_fraction = 0.65
                    if is_pure_bluff:
                        target_bet_fraction = random.uniform(0.55, 0.75)
                    elif is_value_raise:
                        target_bet_fraction = random.uniform(0.60, 0.90)
                    else:
                        target_bet_fraction = random.uniform(0.55, 0.75)
    ...
    # ... (receive_* messages iguales que antes) ...
    ...
    # Placeholder para actualizar stats
